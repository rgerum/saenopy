...
def test_run_example(monkeypatch, random_path, catch_popup_error, use_time, use_reference):
    ...
    if use_time:
        if use_reference:
            file_structure = {
                "run-1": [
                    f"Pos{pos:03d}_S001_z{z:03d}_t{t:02d}_ch{ch:02d}.tif"
                    for z in range(50)
                ...
                "run-1-reference": [
                    f"Pos{pos:03d}_S001_z{z:03d}_ch{ch:02d}.tif"
                    for z in range(50)
                    for pos in range(4, 7)
                    for ch in range(2)
                ],
    ...
    def handle_files(self: AddFilesDialog):
        ...
        if use_reference:
            ...
            self.stack_reference.input_filename.setValue(
                f"run-1-reference/Pos{pos}_S001_z000_ch00.tif", send_signal=True
            )
            if use_channels:
    ...
    print(
        "params A",
        getattr(
            batch_evaluate.sub_module_deformation.result,
    ...
    print(
        "params C",
        getattr(
            batch_evaluate.sub_module_regularize.result,
...
def test_path_editor(monkeypatch, random_path, catch_popup_error, use_time):
    ...
    file_structure = {
        "tmp_path_editor": {
            "run-1": [
                f"Pos{pos:03d}_S001_z{z:03d}_t{t:03d}_ch00.tif"
                for z in range(3)
                for pos in range(4, 7)
                for t in range(3)
            ],
    ...
    def handle_files(self: AddFilesDialog):
        # set the data stack
        self.stack_data.input_filename.setValue(
            "tmp_path_editor/run-1/Pos004_S001_z000_t000_ch00.tif", send_signal=True
        )
        ...
        if use_time:
            ...
            self.reference_choice.setValue(1, send_signal=True)
            self.stack_reference.input_filename.setValue(
                "tmp_path_editor/run-1-reference/Pos004_S001_z000_ch00.tif",
                send_signal=True,
            )
...
def test_copy_paste_params(monkeypatch, random_path, catch_popup_error):
    ...
    def handle_files(self: AddFilesDialog):
        # set the data stack
        self.stack_data.input_filename.setValue(
            "tmp_copy_paste_params/run-1/Pos004_S001_z000_ch00.tif", send_signal=True
        )
        ...
        self.reference_choice.setValue(1, send_signal=True)
        self.stack_reference.input_filename.setValue(
            "tmp_copy_paste_params/run-1-reference/Pos004_S001_z000_ch00.tif",
            send_signal=True,
        )
...
def test_analysis(monkeypatch, catch_popup_error, random_path):
    ...
    for i in range(2):
        ...
        for i in range(2):
            ...
            for button in [
                plotting_window.button_run,
                plotting_window.button_run2,
                plotting_window.button_run3,
            ]:
                button.clicked.emit()
                for prop in properties:
                    plotting_window.type.setValue(prop, send_signal=True)
