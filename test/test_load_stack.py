...
def files_channels(random_path):
    file_structure = {
        "tmp": {
            ...
            "run-2-reference_diff_channels": [
                f"Pos004_S001_z{z:03d}_ch{c:02d}.tif" for z in range(2) for c in range(2)
            ],
        }
    }
    mock_dir(file_structure, callback=lambda file: create_tif(file, x=10, y=20))
...
def files_time(random_path):
    file_structure = {
        "tmp": {
            "run-time": [
                f"Pos004_S001_z{z:03d}_ch{c:02d}_t{t:03d}.tif" for z in range(2) for c in range(3) for t in range(2)
            ],
        }
    }
    mock_dir(file_structure, callback=lambda file: create_tif(file, x=10, y=20))
...
def test_stack_one_channel(files_one_channel):
    ...
    results = get_stacks(
        "tmp/run-1/Pos004_S001_z{z}_ch00.tif",
        "tmp/run-1",
        [1, 1, 1],
    ...
    with pytest.raises(
        ValueError,
        match="if the active stack has channels the reference stack also needs channels",
    ):
        get_stacks(
            "tmp/run-1/Pos004_S001_z{z}_ch{c:00}.tif",
            "tmp/run-1",
            [1, 1, 1],
            reference_stack="tmp/run-1-reference/Pos004_S001_z{z}_ch00.tif",
        )
...
def test_stack_png(files_one_channel):
    ...
    imageio.imwrite(
        "tmp/run-1-png/Pos004_S001_z000_ch00.png",
        (np.random.rand(22, 10) * 255).astype(np.uint8),
    )
    imageio.imwrite(
        "tmp/run-1-png/Pos004_S001_z001_ch00.png",
        (np.random.rand(22, 10) * 255).astype(np.uint8),
    )
    imageio.imwrite(
        "tmp/run-0-png/Pos004_S001_z000_ch00.png",
        (np.random.rand(22, 10) * 255).astype(np.uint8),
    )
    imageio.imwrite(
        "tmp/run-0-png/Pos004_S001_z001_ch00.png",
        (np.random.rand(22, 10) * 255).astype(np.uint8),
    )

    get_stacks(
        "tmp/run-1-png/Pos004_S001_z{z}_ch00.png",
        "tmp/run-1",
        [1, 1, 1],
    ...
    for i in range(2):
        for rep in range(3):
            for z in range(4):
                ...
                imageio.imwrite(
                    f"tmp/run-{i}-glob/Pos{rep:03d}_S001_z{z:03d}_ch00.png",
                    (np.random.rand(22, 10) * 255).astype(np.uint8),
                )

    get_stacks(
        "tmp/run-1-glob/Pos*_S001_z{z}_ch00.png",
        "tmp/run-1-glob",
        [1, 1, 1],
    ...
    for i in range(2):
        for t in range(4):
            for z in range(4):
                ...
                imageio.imwrite(
                    f"tmp/run-{i}-time/Pos001_S001_z{z:03d}_{t:03d}_ch00.png",
                    (np.random.rand(22, 10) * 255).astype(np.uint8),
                )

    get_stacks(
        "tmp/run-1-time/Pos*_S001_z{z}_{t}_ch00.png",
        "tmp/run-1-glob",
        [1, 1, 1],
        crop={"t": (1, None)},
...
def test_overwrite():
    for i in range(2):
        for t in range(3):
            for z in range(4):
                ...
                imageio.imwrite(
                    f"tmp/run-{i}-overwrite/Pos001_S001_z{z:03d}_t{t:03d}_ch00.png",
                    (np.random.rand(22, 10) * 255).astype(np.uint8),
                )
    ...
    for reference_stack in ["tmp/run-0-overwrite/Pos*_S001_z{z}_t000_ch00.png", None]:
        ...
        res = get_stacks(
            "tmp/run-1-overwrite/Pos*_S001_z{z}_t{t}_ch00.png",
            "tmp2/run-1-overwrite",
            [1, 1, 1],
            crop={"t": (1, None)},
        ...
        get_stacks(
            ...
            crop={"t": (1, None)},
            reference_stack=reference_stack,
            exist_overwrite_callback=lambda x: 0,
        )
        ...
        get_stacks(
            ...
            crop={"t": (1, None)},
            reference_stack=reference_stack,
            exist_overwrite_callback=lambda x: "read",
        )
...
def test_stack_channels(files_channels):
    ...
    # check load stack with reference stack
    results = get_stacks(
        "tmp/run-2/Pos004_S001_z{z}_ch{c:00}.tif",
        "tmp/run-1",
        [1, 1, 1],
...
def test_stack_time(files_time):
    # check load stack with reference stack
    results = get_stacks(
        "tmp/run-time/Pos004_S001_z{z}_ch{c:00}_t{t}.tif",
        "tmp/run-1b",
        [1, 1, 1],
        time_delta=1,
    )
...
def test_crop(files_one_channel):
    # crop y
    check_stack(
        Stack("tmp/run-1/Pos004_S001_z{z}_ch00.tif", (1, 1, 1), crop={"y": [5, 15]}),
        10,
        10,
        2,
        1,
    ...
    # crop x
    check_stack(
        Stack("tmp/run-1/Pos004_S001_z{z}_ch00.tif", (1, 1, 1), crop={"x": [2, 8]}),
        20,
        6,
        2,
        1,
...
def test_crop_z(files_z_pages):
    ...
    # crop z
    check_stack(
        Stack("tmp/run-1/Pos004_S001_ch00.tif[z]", (1, 1, 1), crop={"z": [3, 6]}),
        50,
        50,
        3,
        1,
    ...
    results = get_stacks(
        "tmp/run-1/Pos004_S001_ch00.tif[z]",
        "tmp/run-1b",
        [1, 1, 1],
        time_delta=1,
...
def test_lif():
    ...
    results = get_stacks(
        "*/PR2729_frameOrderCombinedScanTypes{f:0}{c:0}.lif",
        "tmp/run-lif",
        [1, 1, 1],
        time_delta=1,
    )
