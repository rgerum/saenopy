...
class PlottingWindow(PlottingWindowBase):
    ...
    def add_parameters(self):
        self.type = QtShortCuts.QInputChoice(
            ...
            [
                "strain_energy",
                "contractility (force center)",
                "contractility (deformations center)",
                "contractility (force center t0)",
                "contractility (deformations center t0)",
                "polarity",
        ...
        self.agg = QtShortCuts.QInputChoice(
            None,
            "aggregate",
            "mean (± sem)",
            ...
            res.resulting_data.append(
                {
                    ...
                    "99_percentile_deformation_µm": np.nanpercentile(
                        np.linalg.norm(
                            M.mesh.displacements_target[M.mesh.regularisation_mask],
                            axis=1,
                        ),
                        99,
                    )
                    * 1e6,
                    "99_percentile_force_nN": np.nanpercentile(
                        np.linalg.norm(M.mesh.forces[M.mesh.regularisation_mask], axis=1),
                        99,
                    )
                    * 1e9,
                    "filename": file,
