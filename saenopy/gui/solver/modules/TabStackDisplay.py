            ...
            with QtShortCuts.QHBoxLayout() as layout:
                with QtShortCuts.QVBoxLayout() as layout:
                    ...
                    with QtShortCuts.QHBoxLayout() as layout:
                        ...
                        self.button_display_single = QtShortCuts.QInputBool(
                            None,
                            "",
                            icon=[
                            ...
                            group=True,
                            tooltip=[
                                "Parallel view of the two stacks",
                                "View only one stack and alternate view between them",
                            ],
                        )
                        ...
                        self.channel_select = QtShortCuts.QInputChoice(
                            ...
                            [0],
                            ["       "],
                            tooltip="From which channel to show.",
                        )
    ...
    def export(self):
        ...
        if new_path:
            ...
            if self.result.stack_reference is not None:
                stack1, stack2 = (
                    self.result.stack_reference[:, :, :, z, c],
                    self.result.stacks[t][:, :, :, z, c],
                )
            else:
                stack1, stack2 = (
                    self.result.stacks[t][:, :, :, z, c],
                    self.result.stacks[t + 1][:, :, :, z, c],
                )
    ...
    def setResult(self, result: Result):
        ...
        if result and result.stacks and result.stacks[0]:
            ...
            if result.stacks[0].channels:
                self.channel_select.setValues(np.arange(len(result.stacks[0].channels)), result.stacks[0].channels)
            ...
            for i in range(2 - self.view_single):
                ...
                try:
                    ...
                    if self.result.stack_parameters["z_project_name"] == "maximum":
                        start = np.clip(
                            z - self.result.stack_parameters["z_project_range"],
                            0,
                            stack.shape[2],
                        )
                        end = np.clip(
