...
def getVectorFieldImage(self, use_fixed_contrast_if_available=False, use_2D=False):
    try:
        ...
        if image and self.t_slider.value() < len(self.result.stacks):
            if (
                getattr(self, "input_reference_stack", None)
                and self.input_reference_stack.value()
                and self.result.stack_reference
            ):
            ...
            display_image = [
                im,
                stack.voxel_size,
                self.z_slider.value() - stack.shape[2] / 2,
            ]
    ...
    try:
            ...
            if name == "forces":
                ...
                if log_scale:
                    ...
                    with np.errstate(divide="ignore", invalid="ignore"):
                        point_cloud.point_data[name + "_mag2"] = np.log10(
                            1e12 * point_cloud.point_data[name + "_mag"].copy()
                        )
        ...
        if show_grid == 1 and (field is not None or stack_min_max is not None):
            ...
            plotter.show_grid(
                ...
                color=plotter.theme.font.color,
                render=False,
            )
    finally:
