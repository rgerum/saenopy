...
class Regularizer(PipelineModule):
    ...
    def __init__(self, parent: "BatchEvaluate", layout):
        ...
        with QtShortCuts.QVBoxLayout(self) as layout:
            ...
            with CheckAbleGroup(
                self,
                "fit forces (regularize)",
                url="https://saenopy.readthedocs.io/en/latest/interface_solver.html#fit-deformations-and-calculate-forces",
            ).addToLayout() as self.group:
                with QtShortCuts.QVBoxLayout() as main_layout:
                    with QtShortCuts.QGroupBox(None, "Material Parameters") as self.material_parameters:
                        with QtShortCuts.QHBoxLayout() as layout2:
                            self.input_k = QtShortCuts.QInputString(
                                None,
                                "k",
                                "1645",
                                type=float,
                                tooltip="the stiffness of the material's fibers",
                            )
                            self.input_d_0 = QtShortCuts.QInputString(
                                None,
                                "d_0",
                                "0.0008",
                                type=float,
                                tooltip="the bluckling strength of the material's fibers",
                            )
                            self.input_lamda_s = QtShortCuts.QInputString(
                                None,
                                "λ_s",
                                "0.0075",
                                type=float,
                                tooltip="the length at which strain stiffening of the material's fibers starts",
                            )
                            self.input_d_s = QtShortCuts.QInputString(
                                None,
                                "d_s",
                                "0.033",
                                type=float,
                                tooltip="the strain stiffening strength of the material's fibers",
                            )

                    with QtShortCuts.QGroupBox(None, "Regularisation Parameters") as self.material_parameters:
                        self.input_previous_t_as_start = QtShortCuts.QInputBool(
                            None,
                            "use previous time steps deformation field",
                            True,
                        ...
                        with QtShortCuts.QHBoxLayout(None) as layout:
                            self.input_alpha = QtShortCuts.QInputString(
                                ...
                                "1e10",
                                type="exp",
                                tooltip="the strength of the regularisation (higher values mean weaker forces)",
                            )
                            self.input_step_size = QtShortCuts.QInputString(
                                None,
                                "step size",
                                "0.33",
                                type=float,
                                tooltip="the step with of the iteration algorithm",
                            )
                        with QtShortCuts.QHBoxLayout(None) as layout:
                            self.input_imax = QtShortCuts.QInputNumber(
                                None,
                                "max iterations",
                                100,
                                float=False,
                                tooltip="the maximum number of iterations after which to abort the iteration algorithm",
                            )
                            self.input_conv_crit = QtShortCuts.QInputString(
                                None,
                                "rel. conv. crit.",
                                0.01,
                                type=float,
                                tooltip="the convergence criterion of the iteration algorithm",
                            )

                    with QtShortCuts.QHBoxLayout():
                        self.input_button = QtShortCuts.QPushButton(
                            None,
                            "calculate forces",
                            self.start_process,
                        ...
                        self.input_button_reset = QtShortCuts.QPushButton(
                            None,
                            "",
                            self.reset,
                            icon=qta.icon("fa5s.trash-alt"),
                            tooltip="reset",
                        )
    ...
    def initialize_plot(self):
        ...
        self.canvas_text = self.canvas.figure.axes[0].text(
            0.5,
            0.5,
            "no fit yet",
            ha="center",
        ...
        self.canvas.figure.axes[0].text(
            1,
            0,
            "\n\niteration",
            ha="right",
    ...
    def process(self, result: Result, material_parameters: dict, solve_parameters: dict):
        ...
        for i in range(len(result.solvers)):
            ...
            if getattr(result.solvers[i], "regularisation_results", None) is not None:
                ...
                if (
                    i < len(result.solvers) - 1
                    and getattr(result.solvers[i + 1], "regularisation_results", None) is not None
                ):
                    # then skip
                    continue
            self.parent.signal_process_status_update.emit(
                f"{i}/{len(result.solvers)} fitting forces",
            ...
            M.set_material_model(
                saenopy.materials.SemiAffineFiberMaterial(
            ...
            M.solve_regularized(
                step_size=solve_parameters["step_size"],
                max_iterations=solve_parameters["max_iterations"],
                alpha=solve_parameters["alpha"],
                rel_conv_crit=solve_parameters["rel_conv_crit"],
                callback=callback,
                verbose=True,
                cancel_signal=self.cancel_p,
            )
        ...
        self.parent.signal_process_status_update.emit(
            f"{i + 1}/{len(result.solvers)} fitting forces",
            f"{Path(result.output).name}",
        )
    ...
    def update_plot(self):
        ...
        if relrec is None:
            relrec = getattr(
                self.result.solvers[self.parent.t_slider.value()],
