...
class DeformationDetector(PipelineModule):
    ...
    def __init__(self, parent: "BatchEvaluate", layout):
        ...
        with QtShortCuts.QVBoxLayout(self) as layout:
            ...
            with CheckAbleGroup(
                self,
                "find deformations (piv)",
                url="https://saenopy.readthedocs.io/en/latest/interface_solver.html#detect-deformations",
                ...
                with QtShortCuts.QVBoxLayout() as layout:
                    ...
                    with QtShortCuts.QHBoxLayout():
                        self.input_signoise = QtShortCuts.QInputNumber(
                            None,
                            "signal to noise",
                            1.3,
                            step=0.1,
                        ...
                        self.input_driftcorrection = QtShortCuts.QInputBool(
                            None,
                            "drift correction",
                            True,
    ...
    def process(self, result: Result, piv_parameters: dict):
        ...
        for i in range(count):
            ...
            self.parent.signal_process_status_update.emit(
                f"{i}/{count} finding deformations", f"{Path(result.output).name}"
            )
            p = ProcessSimple(
                getDeformation,
                (i, result, piv_parameters),
                {},
        ...
        self.parent.signal_process_status_update.emit(
            f"{count}/{count} finding deformations", f"{Path(result.output).name}"
        )
        ...
        def code(my_piv_params):  # pragma: no cover
            ...
            for result in results:
                ...
                for i in range(count):
                    ...
                    result.mesh_piv[i] = saenopy.get_displacements_from_stacks(
                        stack1,
                        stack2,
...
def getDeformation(progress, i, result, params):
    ...
    try:
        if result.stack_reference is None:
            ...
            stack1, stack2 = result.stack_reference, result.stacks[i]
        mesh_piv = saenopy.get_displacements_from_stacks(
            stack1,
            stack2,
            params["window_size"],
            params["element_size"],
