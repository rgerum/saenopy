...
class AddFilesDialog(AddFilesDialog):
        ...
        with self.tabs.createTab("New Measurement") as self.tab:
            with QtShortCuts.QHBoxLayout():
                with QtShortCuts.QVBoxLayout():
                    with QtShortCuts.QHBoxLayout():
                        self.reference_choice = QtShortCuts.QInputChoice(
                            None,
                            "Reference",
                            0,
                            [0, 1],
                            ["difference between time points", "single stack"],
                        )
                    ...
                    self.stack_crop = StackSelectorCrop(
                        self.stack_data, self.reference_choice, self.stack_reference
                    ).addToLayout()
                    self.stack_data.stack_crop = self.stack_crop
                self.stack_preview = StackPreview(
                    QtShortCuts.currentLayout(),
    ...
    def accept_new(self):
        ...
        if self.reference_choice.value() == 1 and self.stack_reference.active is None:
            ...
            QtWidgets.QMessageBox.critical(self, "Deformation Detector", "Enter a valid time delta.")
        else:
            self.accept()
