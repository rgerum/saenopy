...
def sato_filter(data, sigma_sato=None):
    ...
    for z in range(data.shape[-1]):
        fiber[:, :, z] = sato(
            data[:, :, z],
            sigmas=range(sigma_sato, sigma_sato + 1, 1),
            black_ridges=False,
        )
    return fiber
...
def scale_intensity(data, percentile, range):
    ...
    range = (
        original_min + (original_max - original_min) * range[0],
...
def zoom_z(scaled_data, factor):
    if 1:
        pos = np.linspace(0, scaled_data.shape[2] - 1, int(scaled_data.shape[2] * factor))
        f2 = pos % 1
        f1 = 1 - f2
...
def process_stack(
    ...
    return {
        ...
        "center": tuple(center),
        "resolution": tuple(resolution * skip),
    }
...
def join_stacks(stack_data1, stack_data2, thres_cell=1):
    ...
    return {
        ...
        "opacity": opacity,
        "cmap": newcmp,
        "center": stack_data1["center"],
        "resolution": stack_data1["resolution"],
    }
...
class ChannelProperties(QtWidgets.QWidget):
    ...
    def __init__(self, use_channel=False):
        ...
        for widget in [
            self.input_sato,
            self.input_gauss,  # self.input_percentile1, self.input_percentile2,
    ...
    def value(self):
        value = dict(
            show=self.input_show.value(),
            skip=self.input_skip.value(),
            sigma_sato=self.input_sato.value(),
            sigma_gauss=self.input_gauss.value(),
            percentiles=(
                0,
                1,
            ),  # (self.input_percentile1.value(), self.input_percentile2.value()),
        ...
        with self.parent.tabs.createTab("Fiber View") as self.tab:
            with QtShortCuts.QVBoxLayout() as layout:
                self.label_tab = QtWidgets.QLabel(
                    "The deformations from the piv algorithm interpolated on the new mesh for regularisation."
                ).addToLayout()
    ...
    def update_display(self):
        ...
        if self.check_evaluated(self.result):
            ...
            if self.channel0_properties.input_show.value():
                stack_data1 = process_stack(
                    self.result.stacks[t],
                    0,
                    crops=crops,
            ...
            try:
                if stack_data is not None:
                    vol = self.plotter.add_volume(
                        stack_data["data"],
                        resolution=np.array(self.result.stacks[0].voxel_size),
