...
params = {
    ...
    "crop": {"x": (156, 356), "y": (156, 356), "z": (163, 213)},
    "channel0": {
        "show": False,
        "skip": 1,
        "sigma_sato": 2,
...
params = {
    ...
    "channel1": {
        ...
        "percentiles": (0, 1),
        "range": (0, 1),
        "alpha": (0.028954886793518153, 0.5444943820224719, 0.6402247191011237),
        "cmap": "Greens",
        "channel": 1,
    },
    "channel_thresh": 1.0,
...
if 1:
    ...
    for use_log in [False, True]:  # "004", "007",
        ...
        im0 = saenopy.render_image(
            params,
            saenopy.load(folder / f"Pos{pos}_S001_z{{z}}_ch{{c00}}_new14_boundary-False_saenooldbulk.saenopy"),
        )
...
if 1:
    ...
    for pos in group.row(["007"]):  # ["004", "007", "008"]
        ...
        def get_cell_boundary(
            ...
            for i in range(len(result.stacks)):
                ...
                y = np.linalg.norm(
                    result.solvers[0].mesh.forces * result.solvers[0].mesh.regularisation_mask[:, None],
                    axis=1,
                )
                y_proj = np.sum(
                    result.solvers[0].mesh.forces
                    * result.solvers[0].mesh.regularisation_mask[:, None]
                    * difference_vec_normalized,
                    axis=1,
                )
                i = np.argsort(x)
                x = x[i]
    ...
    plt.axhline(5, color="k", linestyle="--", lw=0.8)
