...
class StackSelectorLeica(QtWidgets.QWidget):
    ...
    def setImage(self, filename):
        ...
        self.input_folder.setValues(
            list(np.arange(new.num_images)),
            [im.info["path"] + im.info["name"] for im in new.get_iter_image()],
        )
        self.setFolder(0)
    ...
    def showImage(self):
        ...
        if self.im.scale[2] is None:
            ...
            self.label.setText(
                f"Voxel {self.im.scale[0]:.3}µm x {self.im.scale[1]:.3}µm x {self.im.scale[2]:.3}µm ({im.shape[1]}, {im.shape[0]}, {self.im.dims.z})"
            )
