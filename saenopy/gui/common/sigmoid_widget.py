...
class SpeedSlider:
    def __init__(self, parent, ax, extent, end):
        ...
        self.path3 = matplotlib.patches.Rectangle(
            (extent[0], extent[2]),
            extent[1] - extent[0],
            extent[3] - extent[2],
            facecolor="gray",
    ...
    def update_line(self, hovered, value):
        ...
        if self.end is False:
            self.im3.set_extent(
                [
                    self.centerx - value * 10,
                    self.centerx + 10 - value * 10,
                    self.extent[2],
                    self.extent[3],
                ]
            )
            ...
            self.im3.set_extent(
                [
                    self.centerx - 10 - (value - 1) * 10,
                    self.centerx - (value - 1) * 10,
                    self.extent[2],
                    self.extent[3],
                ]
            )
...
class PolygonInteractor:
    ...
    def __init__(self, parent, ax):
        ...
        (self.line2b,) = ax.plot([0, 1], [1, 1], "o-k", lw=0.8, ms=3)
        ...
        if 0:
            ...
            ax.add_patch(self.path3)
            self.im3 = ax.imshow(
                np.arange(0, 255)[None, :],
                cmap="gray",
                extent=[0, 1, -0.08 - 0.1, -0.02 - 0.1],
                zorder=1,
            )
            self.im3.set_clip_path(self.path3)

            self.path4 = matplotlib.patches.Rectangle(
                (0.5, -0.08 - 0.1), 0.5, 0.06, facecolor="gray", edgecolor="black"
            )
    ...
    def get_range(self):
        ...
        return self.original_min + (self.original_max - self.original_min) * self.minx, self.original_min + (
    ...
    def update_line(self):
        ...
        self.line2a.set_data([[0, 1], [self.a3, self.a3]])
        self.line2b.set_data([[self.a2, self.a2], [0, self.a3 / 2]])
        self.line2c.set_data([[x1, x2], [x1 * m + t, x2 * m + t]])
        if self.hovered == 1:
            self.line2_hover.set_data([[x1, x2], [x1 * m + t, m * x2 + t]])
        if self.hovered == 2:
            self.line2_hover.set_data([[self.a2, self.a2], [0, self.a3 / 2]])
        if self.hovered == 3:
            self.line2_hover.set_data([[0, 1], [self.a3, self.a3]])
        if self.hovered == 0:
    ...
    def get_clicked(self, event):
        ...
        x2 = self.a2 + 0.3 * np.sqrt(1 / (1 + m**2))
        if (
            dist_line_to_point([0, t], [1, m + t], [event.xdata, event.ydata]) < 0.04
            and x1 - 0.04 <= event.xdata < x2 + 0.04
        ):
            return 1
