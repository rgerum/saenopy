...
for pos in ["004", "007", "008"]:
    for a in [2, 3, 10]:
        ...
        target_filename = (
            "/home/richard/.local/share/saenopy/1_ClassicSingleCellTFM/example_output/Pos"
            + pos
            + "_S001_z{z}_ch{c00}_a-"
            + str(a)
            + ".saenopy"
        )
        ...
        if 0:
            for result in results:
                ...
                if saeno != "oldbulk":
                    get_cell_boundary(
                        result,
                        element_size=mesh_parameters["element_size"] * 1e-6,
                        pos=pos,
                        boundary=boundary,
                    )
                result.save()
        ...
        solve_parameters = {
            ...
            "step_size": 0.33,
            "max_iterations": 200,
            "rel_conv_crit": 0.0009,
            "prev_t_as_start": True,
        }
