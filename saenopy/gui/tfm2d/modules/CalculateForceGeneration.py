...
class ForceGeneration(PipelineModule):
            ...
            with CheckAbleGroup(self, "force generation").addToLayout() as self.group:
                with QtShortCuts.QVBoxLayout():
                    self.label = QtWidgets.QLabel(
                        "draw a mask with the red color to select the area where deformations and tractions that are generated by the colony."
                    ).addToLayout()
    ...
    def process(self, result: Result2D, force_gen_parameters: dict):  # type: ignore
        ...
        results_dict = calculate_strain_energy(
            result.mask,
            result.pixel_size,
            result.shape,
            result.u,
