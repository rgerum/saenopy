            ...
            with QtShortCuts.QTabWidget(layout) as self.tabs:
                with self.tabs.createTab("New Measurement") as self.tab:
                    with QtShortCuts.QHBoxLayout():
                        ...
                        with QtShortCuts.QVBoxLayout():
                            ...
                            self.stack_reference_input = QtShortCuts.QInputFilename(None, None, file_type="Image Files (*.tif)", settings=settings,
                                                                     settings_key="2d/input1", allow_edit=True, existing=True)
                        with QtShortCuts.QVBoxLayout():
                            label1 = QtWidgets.QLabel("deformed").addToLayout()
                            self.stack_data_input = QtShortCuts.QInputFilename(None, None, file_type="Image Files (*.tif)", settings=settings,
                                                                     settings_key="2d/input2", allow_edit=True, existing=True)
                    self.pixel_size = QtShortCuts.QInputString(None, "pixel size", 0.201, settings=settings, settings_key="2d/pixel_size", allow_none=False, type=float)
                    QtShortCuts.current_layout.addStretch()
                    self.outputText = QtShortCuts.QInputFolder(None, "output", settings=settings,
                                                               settings_key="2d/wildcard", allow_edit=True)
                    with QtShortCuts.QHBoxLayout():
                ...
                with self.tabs.createTab("Existing Measurement") as self.tab3:
                    self.outputText3 = QtShortCuts.QInputFilename(None, "output", settings=settings,
                                                                  file_type="Results Files (*.saenopy2D)",
                                                                  settings_key="2d/load_existing",
...
class AddFilesDialogX(QtWidgets.QDialog):
    mode: str = None
    mode_data: str = None
    start_time: float = 0
