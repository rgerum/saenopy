...
class CalculateStress(PipelineModule):
            ...
            with CheckAbleGroup(self, "colony stress (optional)").addToLayout() as self.group:
                with QtShortCuts.QVBoxLayout():
                    self.label = QtWidgets.QLabel(
                        "draw a mask with the red color to select an area slightly larger then the colony. Draw a mask with the green color to circle every single cell and mark their boundaries."
                    ).addToLayout()
                    self.label.setWordWrap(True)
                    self.input_button = QtShortCuts.QPushButton(
                        None, "calculate stress & line tensions", self.start_process
                    )
    ...
    def process(self, result: Result2D, stress_parameters: dict):  # type: ignore
        res_dict, attributes = calculate_stress(
            result.mask, result.pixel_size, result.shape, result.u, result.tx, result.ty
        )
