...
from saenopy.gui.common.gui_classes import (
    CheckAbleGroup,
    MatplotlibWidget,
    NavigationToolbar,
)
        ...
        with QtShortCuts.QHBoxLayout(self):
            ...
            with QtShortCuts.QVBoxLayout():
                with QtShortCuts.QHBoxLayout():
                    self.input = QtShortCuts.QInputString(
                        None,
                        "start",
                        str(value),
                        type=float,
...
class MainWindowFit(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        with QtShortCuts.QHBoxLayout(self):
            with QtShortCuts.QVBoxLayout():
                ...
                self.input_type = QtShortCuts.QInputChoice(
                    None,
                    "type",
                    "none",
                    [
    ...
    def run(self):
            ...
            if d[1] and d[2]["type"] != "none":
                ...
                try:
                    ...
                    return QtWidgets.QMessageBox.critical(
                        self, "Fit Error", f"Invalid column specified for file {d[0]}"
                    )
        ...
        params, plot = macro.minimize(parts, param_start, colors=colors)
        params_dict = {k: maps[k](params) for k in maps}
        print(params_dict)
    ...
    def add_file(self, new_path):
        ...
        print(data.shape)
        params = [
            f"k{self.params_index}",
            f"d_0{self.params_index}",
            f"lambda_s{self.params_index}",
            f"d_s{self.params_index}",
        ]
        self.list.addData(
            new_path,
            True,
            dict(data=data, type="none", params=params, transpose=False, col1=0, col2=1),
            mpl.colors.to_hex(f"C{len(self.data)}"),
        )
