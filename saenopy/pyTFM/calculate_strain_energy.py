...
def calculate_strain_energy(mask, pixel_size, shape, u, v, tx, ty):
    mask = binary_fill_holes(mask == 1)  # the mask should be a single patch without holes
    ...
    ps2 = ps1 * np.mean(np.array(shape) / np.array(u.shape))  # pixel size of the deformation field
    # strain energy:
    ...
    # contractility
    contractile_force, proj_x, proj_y, center = contractility(tx, ty, ps2, mask)  # 2.03*10**-6 N
