...
def calculate_stress(mask, pixel_size, shape, u, tx, ty):
    ...
    mask_fem = binary_fill_holes(mask == 1)  # the mask should be a single patch without holes
    ...
    sigma_max, sigma_min, sigma_max_abs, tau_max, phi_n, phi_shear, sigma_mean = all_stress_measures(
        stress_tensor, px_size=ps2 * 10**-6
    )
    mask_int = mask_interpolation(mask_cells_full, dims=sigma_mean.shape, min_cell_size=100)
    ...
    res_dict["cv mean normal stress Cell Area"] = coefficient_of_variation(mask_int, sigma_mean, 0)
    res_dict["cv max normal stress Cell Area"] = coefficient_of_variation(mask_int, sigma_max_abs, 0)
    res_dict["cv max shear stress Cell Area"] = coefficient_of_variation(mask_int, tau_max, 0)
    ...
    lt, min_v, max_v = line_tension(borders.lines_splines, borders.line_lengths, stress_tensor, pixel_length=ps2)
    ...
    res_dict["average magnitude line tension"] = np.mean(np.linalg.norm(lt_vecs, axis=1))
    ...
    avg_cell_force = mean_stress_vector_norm(lt, borders, norm_level="cells", vtype="t_vecs", exclude_colony_edge=True)
    ...
    avg_cell_shear = mean_stress_vector_norm(lt, borders, norm_level="cells", vtype="t_shear", exclude_colony_edge=True)
