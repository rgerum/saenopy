...
class Mesh(Saveable):
    ...
    def __init__(self, nodes: np.ndarray = None, tetrahedra: np.ndarray = None, **kwargs):
    ...
    def set_tetrahedra(self, data: np.ndarray):
        ...
        assert data.max() < self.nodes.shape[0], (
            "Mesh tetrahedron node indices cannot be bigger than the number of vertices."
        )
...
class InvalidShape(Exception):
    ...
    def __init__(self, data, data_shape, target_shape, msg):
        self.text = "{msg} (shape {target_shape}) found shape ({data_shape})".format(
            msg=msg, target_shape=target_shape, data_shape=data_shape
        )
...
def check_tetrahedra_scalar_field(self, data: np.ndarray):
    ...
    if len(data.shape) != 1 or (self.tetrahedra is not None and data.shape[0] != self.tetrahedra.shape[0]):
        raise InvalidShape(
            ...
            target_shape=(self.tetrahedra.shape[0],),
...
def check_node_scalar_field(self, data: np.ndarray):
    ...
    if len(data.shape) != 1 or (self.nodes is not None and data.shape[0] != self.nodes.shape[0]):
        raise InvalidShape(
            data,
            data_shape=data.shape,
            target_shape=(self.nodes.shape[0],),
...
def check_node_vector_field(self, data: np.ndarray):
    ...
    if len(data.shape) != 2 or (self.nodes is not None and data.shape[0] != self.nodes.shape[0]) or data.shape[1] != 3:
        raise InvalidShape(
            data,
            data_shape=data.shape,
            target_shape=(self.nodes.shape[0], 3),
